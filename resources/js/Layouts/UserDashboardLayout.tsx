...
export default function UserDashboardLayout({ title, children }: PropsWithChildren<Props>) {
    ...
    return (
        <>
            ...
            <div className="min-h-screen flex flex-col bg-[#F0FAFA] dark:bg-background">

                {/* ── Full-width top navbar ── */}
                <header className="sticky top-0 z-50 bg-white dark:bg-card border-b border-gray-100 dark:border-border flex-shrink-0 h-16 flex items-center justify-between" style={{ boxShadow: '0 1px 0 #e5e7eb, 0 2px 8px rgba(0,159,187,0.06)' }}>
                    {/* Logo zone — exact same width as sidebar so they align */}
                    <div className="hidden lg:flex w-60 flex-shrink-0 h-full items-center px-5 border-r border-gray-100 dark:border-border">
                        <Link href="/">
                ...
                <div className="flex flex-1 min-h-0">
                    ...
                    <aside className="hidden lg:flex flex-col w-60 flex-shrink-0 bg-white dark:bg-card border-r border-gray-100 dark:border-border sticky top-16 self-start h-[calc(100vh-4rem)] overflow-y-auto z-40">
                        {/* Brand accent stripe */}
                        ...
                        <nav className="flex-1 px-3 pt-4 pb-2">
                            <p className="px-2 mb-2 text-[10px] font-semibold text-gray-400 dark:text-muted-foreground uppercase tracking-wider">Navigation</p>
                                            ...
                                            className={[
                                                'flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 group',
                                                    active
                                                        ...
                                                        : 'text-gray-600 dark:text-foreground hover:bg-brand/5 hover:text-brand-navy dark:hover:text-foreground',
                                            ].join(' ')}
                                        >
                                            <item.icon className={['w-4 h-4 flex-shrink-0 transition-colors', active ? 'text-white' : 'text-gray-400 dark:text-muted-foreground group-hover:text-brand'].join(' ')} />
                                            {item.name}
                        ...
                        {/* Sign out */}
                        <div className="px-3 pb-4 pt-2 border-t border-gray-100 dark:border-border">
                            <Link
                                href={route('logout')}
                                method="post"
                                as="button"
                                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-500 dark:text-muted-foreground hover:bg-red-50 hover:text-red-600 dark:hover:text-red-400 transition-all duration-150 group"
                            >
                                <LogOut className="w-4 h-4 flex-shrink-0 text-gray-400 dark:text-muted-foreground group-hover:text-red-500 transition-colors" />
                                Sign Out
                            </Link>
                        </div>
                    </aside>
                ...
                <nav className="lg:hidden fixed bottom-0 inset-x-0 z-50 bg-white dark:bg-card border-t border-gray-100 dark:border-border shadow-[0_-4px_24px_rgba(0,0,0,0.08)]">
                    <div className="flex">
                        {navItems.map((tab) => {
                                ...
                                <Link
                                    ...
                                    className={[
                                        'flex-1 flex flex-col items-center gap-1 pt-2.5 pb-3 transition-colors',
                                            active ? 'text-brand' : 'text-gray-400 dark:text-muted-foreground hover:text-gray-600 dark:hover:text-foreground',
                                    ].join(' ')}
                                >
