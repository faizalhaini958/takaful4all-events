            ...
            <div className="space-y-4">
                    ...
                    {statCards.map(card => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <p className="text-3xl font-bold tabular-nums text-foreground">{card.count}</p>
                            </button>
                        );
                    })}
                    {/* Checked-in summary card */}
                    <div className="col-span-2 sm:col-span-1 rounded-xl border border-sky-500/30 bg-sky-500/5 p-4">
                        <div className="flex items-center justify-between mb-3">
                            <span className="text-[11px] font-semibold uppercase tracking-wider text-sky-600 dark:text-sky-400">Check-in Rate</span>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="border-b border-border/60 bg-muted/40 hover:bg-muted/40">
                                ...
                                <TableHead className="text-xs font-semibold uppercase tracking-wider text-muted-foreground whitespace-nowrap">Email</TableHead>
                                {showRef     && <TableHead className="text-xs font-semibold uppercase tracking-wider text-muted-foreground whitespace-nowrap">Ref No</TableHead>}
                                {showTicket  && <TableHead className="text-xs font-semibold uppercase tracking-wider text-muted-foreground whitespace-nowrap">Ticket</TableHead>}
                                ...
                                {visibleFields.map(field => (
                                    <TableHead key={field.key} className="text-xs font-semibold uppercase tracking-wider text-muted-foreground whitespace-nowrap">
                                        {field.label_en}
                                    </TableHead>
                                ))}
                            </TableRow>
                                ...
                                attendees.data.map((attendee, i) => {
                                    ...
                                    return (
                                        <TableRow key={attendee.id}>
                                            <TableCell className="text-center text-xs text-muted-foreground tabular-nums">{rowNum}</TableCell>
                                            <TableCell>
                                            ...
                                            {visibleFields.map(field => {
                                                ...
                                                return (
                                                    <TableCell key={key} className="text-xs max-w-[200px] truncate text-muted-foreground" title={String(value)}>
                ...
                <div className="flex items-center justify-between rounded-xl border border-border/60 bg-card px-4 py-3 shadow-sm">
                    <span className="text-sm text-muted-foreground">
                        Showing <span className="font-medium text-foreground">{attendees.from ?? 0}</span> to <span className="font-medium text-foreground">{attendees.to ?? 0}</span> of <span className="font-medium text-foreground">{attendees.total}</span>
                    </span>
                    <div className="flex items-center gap-3">
                        <span className="text-sm text-muted-foreground">
